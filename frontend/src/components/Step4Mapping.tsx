      ...
      >
        {currentSlide.shapes.map((shape) => {
          ...
          return (
            <div
              key={shape.index}
              className={`absolute transition-all select-none overflow-hidden placeholder-box ${
                isImagePlaceholder
                  ? shape.imageUrl
    ...
    <div className="space-y-2 max-w-full mx-auto h-[calc(100vh-140px)] flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 overflow-hidden">
        ...
        <div className="lg:col-span-2 surface-card p-3 flex flex-col overflow-hidden">
          <h3 className="text-xs font-bold text-[var(--color-navy)] border-b border-[var(--color-border)] pb-2 mb-3 flex items-center space-x-1.5">
            <LayoutGrid className="w-4 h-4 text-[var(--color-amber)]" />
            <span>Thumbnails ({slides?.length || 0})</span>
          </h3>
            ...
            {slides?.map((slide, idx) => {
              ...
              const hasMissingFigures = slide.shapes.some((shape: any) => {
                ...
                const text = (shape.textBody?.paragraphs || [])
                  ...
                  .join(' ')
                  .trim();
                return /(figure|table|chart)\s+\d+/i.test(text) || /insert\s+(figure|table|chart|image)/i.test(text);
              });
              ...
              return (
                <button
                  ...
                  className={`w-full flex flex-col p-2 border rounded-[var(--radius-custom)] text-left hover:bg-[var(--color-cream)] transition-all ${
                    active
                      ? "border-[var(--color-amber)] bg-[var(--color-cream)] ring-2 ring-[var(--color-amber)]/25"
                      : "border-[var(--color-border)] bg-white"
                  }`}
                >
                  <div className="w-full aspect-[16/9] border bg-white rounded flex items-center justify-center relative overflow-hidden mb-1 shadow-xs select-none pointer-events-none">
                    <div 
                      className="absolute origin-top-left"
                      style={{
                        width: '960px',
                        height: '540px',
                        transform: `scale(${112 / 960})`,
                      ...
                      {slide.shapes.map((shape) => {
                        ...
                        return (
                          <div
                            ...
                            style={{
                              left: `${shape.position.x_pt}px`,
                              top: `${shape.position.y_pt}px`,
                              width: `${shape.size.width_pt}px`,
                              height: `${shape.size.height_pt}px`,
                              ...bgStyle,
                              border: shape.imageUrl ? '2px solid #10b981' : 'none',
                            }}
                          >
                            {shape.imageUrl && (
                              <img src={shape.imageUrl} alt="" className="w-full h-full object-contain" />
                            )}
                            {!shape.imageUrl && isTitle && (
                              <div className="w-full h-full bg-zinc-200/40"></div>
                            )}
                          </div>
                        );
                      })}
                  ...
                  <div className="w-full min-w-0 flex items-center justify-between">
                    <p className="text-[10px] font-semibold text-[var(--color-navy)] truncate flex-1">
                      {slide.shapes.find((s) => s.shapeType && s.shapeType.includes('Title'))?.text || `Slide ${idx + 1}`}
                    </p>
                    {hasMissingFigures && (
        ...
        <div
          ...
          className={`surface-card p-4 flex flex-col justify-between items-center overflow-hidden transition-all duration-300 ${
            showModifications ? 'lg:col-span-7' : 'lg:col-span-10'
          }`}
        >
          {/* Controls header */}
